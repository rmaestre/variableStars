---
title: "Simple Convolutional Autoencoder on the MNIST"
...
modelConv <-  keras_model_sequential() %>%
  # Encoder
  layer_conv_1d(8, kernel_size=2, activation='relu', padding='same',input_shape=c(input_dim, 1), name="a") %>%
  layer_max_pooling_1d(2, name="b") %>%
  layer_conv_1d(4, kernel_size=2, activation='relu', padding='same', name="c")  %>%
  layer_max_pooling_1d(2, name="hidden") %>%
  # Decoder
  layer_conv_1d(4, kernel_size=2, activation='relu', padding='same', name="e")  %>%
  layer_upsampling_1d(2, name="f") %>%
  layer_conv_1d(8, kernel_size=2, activation='relu', padding='same', name="g") %>%
  layer_upsampling_1d(2, name="h") %>%
  layer_conv_1d(1, kernel_size=2, activation='relu', padding='same', name="i") 
```
...
summary(modelConv) # Plot summary
```
...
modelConv %>% evaluate(x_test, x_test, verbose = 1)
```

### Get latent components (i.e: the hidden layer)

```{r autoencoderHidden}
hidden_layer <- keras_model(inputs = modelConv$input,
                            outputs = modelConv$get_layer("hidden")$output)
```
...
latent_hat <- predict(hidden_layer, x_test)
